			...
			function resetHandler() {
				...
				for (var i = 0; i < labRows.length; i++) {
					...
					for (var j = 0; j < labRow.children.length; j++) {
						...
						if(child.children && child.children.length != 0 && child.children[0].rows) {	
							var childRows = child.children[0].rows[0].children;
							for (var k = 0; k < childRows.length; k++) {
								if (childRows[k].getAttribute("style")) {
					...
					<p id="header">
          			...
          			<p>
          				<form name="searchForm" method="GET" onsubmit="return validate();">
          					 ...
          					 <input type="reset" name="reset" onclick="resetHandler();" value="Clear Search" /> 
          				</form>
          			<p ><hr size="4" />
          			<p><font size="-1">
